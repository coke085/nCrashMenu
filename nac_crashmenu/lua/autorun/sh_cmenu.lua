...
if SERVER then
	...
	timer.Create( "tool_list_check", 120, 0, check_tool_list)
...
function CM.CrashMenu()
	...
	for i = 1, (bNum + 2) do	
		...
		CMR.Paint = function()	
			...
			if i == (bNum + 1) then
				draw.SimpleText("Reconnecting in: ", "CMButton2", xRes(15), yRes(22), ButtonTextColor, TEXT_ALIGN_LEFT)
				draw.SimpleText(tostring(math.Round(retryTime - CurTime())).." seconds", "CMButton2", xRes(15), yRes(52), ButtonTextColor, TEXT_ALIGN_LEFT)
		...
		CMR.DoClick = function()
			...
			if i <= bNum then
				if string.find(ServerIPButtons[i], "https://", 0, false) then
